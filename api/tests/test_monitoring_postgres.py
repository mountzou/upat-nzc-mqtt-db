        ...
        with conn.cursor() as cur:
            cur.execute('''DROP SCHEMA IF EXISTS monitoring_fixture CASCADE; CREATE SCHEMA monitoring_fixture;
                ...
                CREATE TABLE shelly_pro3em_hourly_energy(device_id TEXT,window_start TIMESTAMPTZ,window_end TIMESTAMPTZ,
                  ...
                  total_energy_wh DOUBLE PRECISION,is_working_day INTEGER,is_working_hour INTEGER,created_at TIMESTAMPTZ);
            ''')
...
def insert_day(connect,day,power=6,missing=()):
    start=datetime.combine(day,time.min,pv.ATHENS).astimezone(timezone.utc)
    end=datetime.combine(day+timedelta(days=1),time.min,pv.ATHENS).astimezone(timezone.utc)
...
def test_real_sql_daily_hourly_dst(database,client,day,hours):
    ...
    url=f'/energy/production/history?start_date={day}&end_date={day}'
    daily=client.get(url+'&interval=day',headers=headers)
    ...
    hourly=client.get(url+'&interval=1h',headers=headers)
    assert hourly.status_code==200,hourly.text
    values=hourly.json()['points']
    assert len(values)==hours and len({v['start'] for v in values})==hours
    assert all(v['energy_kwh']==6 for v in values)
...
def test_gap_zero_and_missing_preserved(database,client):
    ...
    data=client.get('/energy/production/history?start_date=2026-09-01&end_date=2026-09-03&interval=1h',headers=login(client)).json()
    a,b,c=data['days']
    assert a['quality']=='partial' and a['energy_kwh']==138 and data['points'][0]['energy_kwh'] is None
...
def test_auth_scope_revocation_and_internal_verifier(database,client):
    ...
    assert client.get('/catalog/schools',headers=headers).json()[0]['id']=='school_10'
    assert client.get('/catalog/schools/school_10/rooms',headers=headers).status_code==200
...
def test_hourly_cache_serves_daily_and_does_not_prefetch_hours(database,client):
    ...
    with patch.object(pv,'query_days',wraps=pv.query_days) as query:
        one=pv.get_energy(day,day,'day')
        pv.get_energy(day,day,'day');assert query.call_count==1
...
def test_municipality_and_admin_catalog_scope(database,client):
    ...
    for username,expected in [('fixture_municipality',{'school_10','school_3'}),('fixture_admin',None)]:
        ...
        schools=client.get('/catalog/schools',headers=headers)
        assert schools.status_code==200
        if expected:
            assert {s['id'] for s in schools.json()}==expected
            assert client.get('/catalog/schools/school_22/rooms',headers=headers).status_code==403
        else:assert len(schools.json())>2
        assert client.get('/energy/production/history?start_date=2026-09-01&end_date=2026-09-01&interval=day',headers=headers).status_code==200
