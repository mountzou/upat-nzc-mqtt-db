...
def iter_home_overview(school_id: str, room_devices: dict, refresh=False):
    ...
    def load(device, kind):
        loader = (lambda: get_device_history(device, aggregate='avg', bucket_unit='minute', bucket_size=1, limit=1)) if kind == 'live' else (lambda: get_device_rolling_twenty_four_hour_hourly_series(device))
